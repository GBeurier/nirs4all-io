...
| C ABI | `n4io_abi_version()` / `N4IO_ABI_VERSION` (capi) | `0.0.1` | Independent of crate semver (D-R6). MAJOR bumps on any breaking ABI change; MINOR on additive symbols; PATCH on fixes. Pre-1.0 every breaking change bumps MAJOR. Checked by `n4io_check_abi_compatibility`. |
...
3. **Crate semver ≠ ABI version.** The cargo workspace version
   (`0.1.0-alpha.0`) tracks the Rust crates; the C ABI tracks
