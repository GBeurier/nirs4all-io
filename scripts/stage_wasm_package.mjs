...
const pkgDir = path.resolve(process.argv[2] ?? path.join(root, "bindings/wasm/pkg-node"));
...
for (const name of ["LICENSE", "LICENSING.md", "THIRD_PARTY_NOTICES.md", "COPY_PROVENANCE.md"]) {
  fs.copyFileSync(path.join(root, name), path.join(pkgDir, name));
}
for (const name of fs.readdirSync(path.join(root, "LICENSES")).sort()) {
  fs.copyFileSync(path.join(root, "LICENSES", name), path.join(pkgDir, "LICENSES", name));
}
